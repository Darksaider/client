  ...
  useEffect(() => {
    ...
    fields.forEach((field) => {
      ...
      if (field.type === "select") {
        // UPDATED: Handle simplified structure - now just direct id and name
        if (fieldName === "product_brands") {
          // Now expecting: product_brands: [{id: 1, name: "Brand"}]
          const brandId = rawValueFromProduct?.id;
          if (brandId !== undefined) {
            processedDefaults[fieldName] = brandId as PathValue<T, Path<T>>;
          }
        } else if (fieldName === "product_discounts") {
          // Now expecting: product_discounts: [{id: 1, name: "Discount"}]
          const discountId = rawValueFromProduct?.discounts.id;

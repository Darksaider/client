...
export const AdminProducts: React.FC = () => {
  ...
  const handleUpdateSubmit = useCallback(
    async (productId: number, data: Product) => {
      try {
  ...
  const getBaseProductFormFields = (): FormField<Product>[] => [
    {
      name: "name",
      label: "Назва",
      type: "text",
  ...
  const photoFieldForCreate: FormField<Product> = {
    name: "product_photos",
    ...
    <div className="p-4 mx-auto max-w-7xl">
      ...
      {showCreateForm && (
        <div className="mb-6 border p-4 rounded-lg bg-white shadow-md">
          <GenericForm<Product>
            key="create-form"
            defaultValues={initialProductValues as Product}
      ...
      <PaginationButtons
        productNumber={productData?.totalCount || 0}
        isLoading={isLoadingProducts}
      />
    </div>
  );
